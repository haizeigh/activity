...
CActivityRecognition::CActivityRecognition(SParameters _param)
{
	setParam(_param);
	if (_param.sClassifierType == "NN")
	{
...
void CActivityRecognition::loadTrainData(const std::string str)
{
	...
	IoHdf5.loadDataFromHDF5(  m_basepath  + "/train/"  + str);
...
void CActivityRecognition::setParam(SParameters param)
{
...
void CActivityRecognition::loadModelFromFile(const std::string _str)
{
...
void CActivityRecognition::setTrainData()
{
	if (m_sClassifierType.compare("NN") == 0)
	{
		NNEngine->setTrainData(m_mTrainMat,m_vTrainLabels);
	}
}

// ����ֵ��
// -1 ��ʶ��״̬ 0 walking 1 running 2 sitting
